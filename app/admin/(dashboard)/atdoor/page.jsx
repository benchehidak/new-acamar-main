...
//   const userInput = { seller: '', release: '' };
//   return (
...
export default function SellPage() {
    const [userInput, setUserInput] = useState({ email: '',  seller: '', release: '' });
    ...
    const clearForm = () => {
        setUserInput({  email: '',seller: '', release: '' });
    };
    const closeModal = () => {
    ...
    const handleSubmit = async (e) => {
        ...
        setBtnEnabled(false);
        try {
            const res = await axios.post('/api/protected/atdoor', userInput);
            console.log(res);
        ...
        <div className='sm:rounded-5xl -mx-4 flex-auto  px-4 py-10 sm:mx-0 sm:flex-none sm:p-24'>
          <form onSubmit={handleSubmit} >
            ...
            <div>
                ...
                onChange={(e) => {setUserInput({...userInput, release: e.value , seller: session?.user?.name});
              }}
                />
            </div>
            </div>
